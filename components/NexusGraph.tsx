...
export default function NexusGraph() {
    const { nexusGraphData, isAiAvailable, promptSettings } = useSpudHub();
    ...
    const handleAnalyzeGraph = useCallback(async () => {
        ...
        setInsights('');
        try {
            const result = await getNexusInsights(nexusGraphData.nodes, nexusGraphData.links, promptSettings.getNexusInsights);
            setInsights(result);
        } catch (e) {
    ...
    }, [isAiAvailable, nexusGraphData, promptSettings, addToast]);
